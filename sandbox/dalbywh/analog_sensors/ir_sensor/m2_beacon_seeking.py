...
def seek_beacon(robot):
    ...
    while not robot.touch_sensor.is_pressed:
        ...
        current_heading = beacon_sensor.heading  # use the beacon_seeker heading
        current_distance = beacon_sensor.distance  # use the beacon_seeker distance
        if current_distance == -128:
            ...
            if math.fabs(current_heading) < 2:
                ...
                if current_distance == 0:
                    robot.right_forward(True, forward_speed)
                    robot.left_forward(True, forward_speed)
                    time.sleep(0.4)
                    robot.stop()
                    return True
                ...
                if current_heading < 0:
                    ...
                    robot.right_forward(True, turn_speed)
                    robot.left_backward(True, turn_speed)
                else:
                    print("Beacon is on the right.")
                    robot.right_backward(True, turn_speed)
                    robot.left_forward(True, turn_speed)
