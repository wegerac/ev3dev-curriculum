...
#
#       speed_in_inches_per_second = .0113 * speed_in_degrees_per_second + 0
#
...
while time_s != 0:
    speed = float(input('Enter a speed (0 to 900 dps):'))
    distance = float(input('Distance to travel (inches): '))

    left_motor.run_forever(speed_sp=speed)
    right_motor.run_forever(speed_sp=speed)
    time.sleep(distance/(speed*0.0113))
    left_motor.stop()
