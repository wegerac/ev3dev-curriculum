...
Authors: David Fisher and William Dalby, Michael Kelly, and Andrew Weger.
...
def arm_calibration(arm_motor, touch_sensor):
    ...
    #Code that attempts to do this task but has MANY bugs (nearly 1 on every
    #  line).  Fix them!
    ...
    ev3.Sound.beep()
    arm_revolutions_for_full_range = -5100
    arm_motor.run_to_rel_pos(
        speed_sp=MAX_SPEED, position_sp=arm_revolutions_for_full_range)
...
def arm_up(arm_motor, touch_sensor):
    ...
    arm_motor.run_to_rel_pos(position_sp=5100, speed_sp=MAX_SPEED)
    while not touch_sensor.is_pressed:
        time.sleep(0.01)
    arm_motor.stop()
    ev3.Sound.beep()
...
def arm_down(arm_motor):
    ...
    arm_motor.wait_while(ev3.Motor.STATE_RUNNING)
    ev3.Sound.beep()# Blocks until the motor
    # finishes running
